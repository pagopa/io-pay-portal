...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const mobileViewport = getComputedStyle(document.documentElement).getPropertyValue('--breakpoint-sm');
  ...
  donationByApp?.addEventListener("click", async (e) => {
    e.preventDefault();
    const isMobileDevice = window.matchMedia(`only screen and (max-width: ${mobileViewport})`).matches;
    const toUrl = (e.currentTarget as HTMLElement).getAttribute("data-url") || "";
